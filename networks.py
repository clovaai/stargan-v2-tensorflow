...
class Generator(tf.keras.Model):
    def __init__(self, img_size=256, img_ch=3, style_dim=64, max_conv_dim=512, sn=False, name='Generator'):
        ...
        self.from_rgb = Conv(channels=self.channels, kernel=3,
                             stride=1, pad=1, sn=self.sn, name='from_rgb')
    ...
    def architecture_init(self):
        ...
        for i in range(self.repeat_num):
            ...
            encoder.append(ResBlock(ch_in, ch_out, normalize=True, downsample=True,
                           sn=self.sn, name='encoder_down_resblock_' + str(i)))
            decoder.insert(0, AdainResBlock(ch_out, ch_in, upsample=True,
                           sn=self.sn, name='decoder_up_adaresblock_' + str(i)))  # stack-like
        ...
        for i in range(2):
            encoder.append(ResBlock(ch_out, ch_out, normalize=True,
                           sn=self.sn, name='encoder_bottleneck_resblock_' + str(i)))
            decoder.insert(0, AdainResBlock(
                ch_out, ch_out, sn=self.sn, name='decoder_bottleneck_adaresblock_' + str(i)))
...
class StyleEncoder(tf.keras.Model):
    ...
    def architecture_init(self):
        ...
        blocks += [Conv(ch_in, kernel=3, stride=1, pad=1,
                        sn=self.sn, name='init_conv')]
        ...
        for n_d in range(self.num_domains):
            unshared_layers += [FullyConnected(
                units=self.style_dim, sn=self.sn, name='domain_{}_style_fc'.format(n_d))]
